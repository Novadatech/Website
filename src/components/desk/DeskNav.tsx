...
export default function DeskNav() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[#E2E7EE] bg-white/95 backdrop-blur-md">
      <div className={`${CONTAINER} ${SECTION}`}>
        <div className="flex h-16 md:h-20 items-center justify-between gap-4">
          <Link href="/" className="flex items-center gap-2.5 flex-shrink-0">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/favicon.png" alt="" className="h-7 w-7 md:h-8 md:w-8" />
            <span className="text-lg md:text-xl font-bold tracking-tight text-[#0E1116] leading-none">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              aria-expanded={open}
              onClick={() => setOpen((v) => !v)}
              className="lg:hidden p-2 -mr-2 text-[#0E1116]"
            >
      ...
      {open && (
        <div className="lg:hidden border-t border-[#E2E7EE] bg-white">
          <div className={`${CONTAINER} ${SECTION} py-4 flex flex-col gap-1`}>
            {NAV_ITEMS.map((item) => (
              <Link
                ...
                className="py-3 text-base font-medium text-[#0E1116] border-b border-[#F4F6FA]"
              >
                {item.label}
              </Link>
            ))}
